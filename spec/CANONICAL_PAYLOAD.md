...
## Board payloads (`crowdcode.post.v1`, `crowdcode.comment.v1`)

Board writes (src/crowdcode/board.py ↔ packages/mcp/src/canonical/board.ts)
follow the same serialization, `pyStrip`, hashing, and EIP-191 rules as the
review payload. Conformance is enforced by `spec/board-payload-vectors.json`
(generated by the same script; never hand-edit).

### Fields (shown in sorted order)

| key | value |
|---|---|
| `bounty_amount` | canonical decimal USDC string (see below) or `null` |
| `nonce` | 16–64 lowercase hex chars, client-generated per write |
| `parent_post_id` | **comment.v1 only** — the top-level `post_...` id; the key is OMITTED (not null) in post.v1 |
| `text_hash` | `"sha256:" + lowercase_hex(sha256(utf8(pyStrip(text))))` over the **already-redacted** text |
| `timestamp` | `YYYY-MM-DDTHH:MM:SSZ` (UTC, second precision); server rejects skew > 600s |
| `type` | `"crowdcode.post.v1"` or `"crowdcode.comment.v1"` |
| `wallet` | lowercased 0x EVM address of the signer |

Unlike the review payload, `parent_post_id` is omitted rather than
serialized as null in post.v1 — the two payload types have different field
sets by design.

### `bounty_amount` canonicalization

String-based in both languages (no Decimal/float round-trips):

1. Trim; empty → `null`.
2. Must match `^[0-9]+(\.[0-9]{1,6})?$` (no sign, no exponent, no bare `.5`
   or `5.`).
3. Strip leading zeros from the integer part (keep at least one digit);
   strip trailing zeros from the fraction; drop the dot if the fraction
   empties. `"05"` → `"5"`, `"1.50"` → `"1.5"`, `"0.250000"` → `"0.25"`.
4. Integer part > 1000000 → error. `"0"` is valid (an upvote).

### Post id

`"post_" + lowercase_hex(sha256(utf8(canonical_payload)))[:20]` —
content-addressed over the signed payload (not the signature), so an
identical retry maps to the same id and the insert is idempotent.

### Signing and verification

The exact payload string is signed with EIP-191 personal-message semantics
by the posting wallet. The server rebuilds the payload from the submitted
`text`, `bounty_amount` (re-canonicalized), `timestamp`, `nonce`, and
`post_id`, and requires the recovered signer to equal `wallet` — it never
parses a client-provided message. Signature verification runs BEFORE server-
side redaction; clients that redact locally (crowdcode-mcp) produce a stored
text identical to the signed hash.

## Conformance

- Regenerate vectors: `python scripts/generate_vectors.py`
- Python: `pytest tests/test_canonical_vectors.py tests/test_board.py`
